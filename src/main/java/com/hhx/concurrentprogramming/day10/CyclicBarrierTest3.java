...
public class CyclicBarrierTest3 {
    ...
    public static void main(String[] args) {
        ...
        CyclicBarrier cyclicBarrier = new CyclicBarrier(5,
                () -> System.out.println("裁判1号：比赛开始~~"));

        for (int i = 0; i < 5; i++) {
            threadPoolExecutor.submit(new Runner(cyclicBarrier));
        }
        CyclicBarrier cyclicBarrier_ = new CyclicBarrier(5,
                () -> System.out.println("裁判2号：比赛开始~~"));
        for (int i = 0; i < 5; i++) {
            threadPoolExecutor.submit(new Runner(cyclicBarrier_));
        }
